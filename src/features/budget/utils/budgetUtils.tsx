...
export const generateRecommendations = (budgetComparison, healthScore) => {
    ...
    Object.entries(budgetComparison).forEach(([category, data]) => {
      if (data.status === "over") {
        recommendations.push({
          type: "alert",
          category,
          message: `${category} is R$${Math.abs(data.remaining).toFixed(0)} over budget`,
  ...
  if (score >= 80) {
    ...
    recommendations.push({
      type: "tip",
      category: "Geral",
      message: "Boa saúde financeira, com espaço para melhorar",
      action: "Foque em aumentar a poupança e construir uma reserva de emergência",
    });
  } else {
    recommendations.push({
      type: "warning",
