...
export const CSVImportExport = ({ data, onImport, filteredData }: CSVImportExportProps) => {
  ...
  const handleFileSelect = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      setImportStatus({ type: "loading", message: "Importando CSV..." });
      ...
      setImportStatus({
        type: "error",
        message: (error as Error).message || "Falha ao importar CSV",
      });
      setTimeout(() => setImportStatus(null), 5000);
    }
  ...
  const handleExport = (useFiltered: boolean = false) => {
    try {
      ...
      setExportStatus({
        type: "error",
        message: (error as Error).message || "Falha ao exportar CSV",
      });
      setTimeout(() => setExportStatus(null), 3000);
    }
  };
      ...
      <div className="flex flex-wrap gap-4">
        ...
        <div className="flex-1 min-w-[200px]">
          <button
            ...
            aria-label="Exportar todos os lançamentos em CSV"
          >
            <Download size={20} />
            Export All
          </button>
