 ...
 * formatCurrency(1234567, { compact: true }) // "R$12.35L"
 */
 ...
 * @returns Compact formatted string (e.g., "R$12.35L", "R$1.5Cr")
 * @example
 * formatCompactCurrency(125000) // "R$1.25L"
 * formatCompactCurrency(10000000) // "R$1.00Cr"
 */
export const formatCompactCurrency = (value: number, showSymbol: boolean = true): string => {
  ...
  if (absValue >= 1000000000) {
    // Bilhões
    return `${prefix}${(value / 1000000000).toFixed(2)}bi`;
  } else if (absValue >= 1000000) {
    // Milhões
    return `${prefix}${(value / 1000000).toFixed(2)}mi`;
  } else if (absValue >= 1000) {
    // Milhares
    return `${prefix}${(value / 1000).toFixed(2)}mil`;
  }
 ...
 * @example
 * formatCurrencyWithDecimals(1234.5678, 0) // "R$1,235"
 */
