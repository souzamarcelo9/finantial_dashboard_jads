...
const analyzeDeliverySpending = (
  ...
  return {
    ...
    message: `You order food ${ordersPerWeek.toFixed(1)} times per week (avg R$${avgPerOrder.toFixed(0)} per order). Reducing by 30% could save R$${savings.monthlySavings.toFixed(0)} per month (R$${savings.annualSavings.toFixed(0)} per year)`,
    actionable: true,
    category: "Food",
  };
};
...
const analyzeWeekendPattern = (transactions: Transaction[]): InsightItem | null => {
  ...
  return {
    ...
    message: `You spend ${((avgWeekend / avgWeekday - 1) * 100).toFixed(0)}% more on weekends (R$${avgWeekend.toFixed(0)}/day) compared to weekdays (R$${avgWeekday.toFixed(0)}/day)`,
    actionable: false,
  };
};
...
const analyzeHighFrequencyCategory = (
  ...
  return {
    ...
    priority: "low",
    icon: "🔄",
    title: "Categoria de alta frequência",
    message: `Você realizou ${topCategory[1]} transação(ões) em ${topCategory[0]} (${(frequency * 7).toFixed(1)} vezes por semana na média)`,
    actionable: false,
...
const analyzeCafeteriaSpending = (
  ...
  return {
    ...
    message: `You eat at the office cafeteria ${perDay.toFixed(1)} times per day (avg R$${avgAmount.toFixed(0)} per meal). Packing lunch 50% of the time could save R$${packLunchSavings.monthlySavings.toFixed(0)} per month`,
    actionable: true,
    category: "Food",
  };
};
